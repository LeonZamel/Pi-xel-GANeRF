...
from encoder_with_projection import PixelNeRFEncoder

import numpy as np

# TODO hardcoded for now for carla dataset
def get_camera_info(ex_pose, batch_size, device, normalize=False):
    # intrinsic_matrix = torch.tensor(
    #     [[955.40500674, 0.0, 256.0], [0.0, 955.40500674, 256.0], [0.0, 0.0, 1.0]],
    #     device=device,
    # )
    if normalize:
        focal = torch.tensor([1 / np.tan(np.radians(30 / 2))], device=device)
        C = torch.tensor([256], device=device)
        camera_pos = ex_pose[:, :3, 3]
        normalized_camera_pos = normalize_vecs(camera_pos)
        ex_pose[:, :3, 3] = normalized_camera_pos
        # return focal, C, ex_pose
        return focal, None, ex_pose
    else:
        return torch.tensor([955.40500674/16], device=device), torch.tensor([256.0/16], device=device), ex_pose
...
class ImplicitGenerator3d(nn.Module):
    def __init__(self, siren, z_dim, **kwargs):
        ...
        z_dim = self.z_dim + 512 + 13
        if self.use_global_encoder:
            z_dim += 128

        # fix input dim
        self.siren = siren(
            output_dim=4, z_dim=z_dim, input_dim=3, device=None
        )
        self.epoch = 0
        self.step = 0

        # PixelNeRF stuff
        self.pixel_nerf_encoder = PixelNeRFEncoder(stop_encoder_grad=True, use_global_encoder=self.use_global_encoder)
    ...
    def forward(
        ...
        **kwargs
        ...
        focals, C, ex_poses = get_camera_info(ex_poses, batch_size, self.device, normalize=True)
        focals *= images.shape[-1] / 2 # Very bad, should take into account differing h and w
        with torch.no_grad():
            self.pixel_nerf_encoder.encode(images, ex_poses, focals, c=C)

        pixel_nerf_features = self.pixel_nerf_encoder(transformed_points)
        # TODO: This might be wrong because of the repeat-interleave thingy
        pixel_nerf_features = pixel_nerf_features.reshape(
            (*transformed_points.shape[:2], -1)
        )
        z = z.unsqueeze(1).repeat(1, pixel_nerf_features.shape[1], 1)

        # Model prediction on course points
        coarse_output = self.siren(
            # torch.cat((pixel_nerf_features, transformed_points), dim=-1),
            transformed_points,
            # z,
            torch.cat((z, pixel_nerf_features), dim=-1),
        ...
        if hierarchical_sample:
            ...
            fine_output = self.siren(
                # torch.cat((pixel_nerf_features, fine_points), dim=-1),
                fine_points,
                # z,
                torch.cat((z, pixel_nerf_features), dim=-1),
                ray_directions=transformed_ray_directions_expanded,
        ...
        return pixels, torch.cat([pitches, yaws], -1)
