...
class ImplicitGenerator3d(nn.Module):
    def __init__(self, siren, z_dim, **kwargs):
        ...
        # fix input dim
        self.siren = siren(
            output_dim=4, z_dim=self.z_dim + 512 + 13, input_dim=3, device=None  # 554,
        )
        self.epoch = 0
        self.step = 0

        # PixelNeRF stuff
        self.pixel_nerf_encoder = PixelNeRFEncoder(stop_encoder_grad=True)
    ...
    def forward(
        ...
        **kwargs
        ...
        focals *= images.shape[-1] / 2 # Very bad, should take into account differing h and w5
        with torch.no_grad():
