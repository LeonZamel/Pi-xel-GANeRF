...
class TALLSIREN(nn.Module):
    ...
    def __init__(
        self, input_dim=2, z_dim=100, hidden_dim=256, output_dim=1, device=None, enable_viewdir=False
    ):
        super().__init__()
        print("TALLSIREN", enable_viewdir)
        self.device = device
        ...
        self.enable_viewdir = enable_viewdir
        ...
        self.color_layer_sine = FiLMLayer(hidden_dim + (3 if enable_viewdir else 0), hidden_dim)
    ...
    def forward_with_frequencies_phase_shifts(
        ...
        input_color_net = torch.cat([ray_directions, x], dim=-1) if self.enable_viewdir else x
